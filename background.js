...
browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  if (changeInfo.status === "complete" && tab.url) {
    ...
    if (settings.autoSummarize === true && isArticlePage(tab.url)) {
...
async function handleSummarize(tab, isSelection = false, selectionText = null, isAuto = false) {
  ...
  try {
    ...
    console.log("Dane wysłane do panelu, generuję podsumowanie z Perplexity...");
    ...
    console.log("Podsumowanie gotowe");
...
async function generateSummaryWithPerplexity(data, settings) {
  ...
  if (settings.summaryMode === "long") {
    lengthHint = "w 8–10 zdaniach, dość szczegółowo";
    maxTokens = 700;
  } else {
    lengthHint = "w 4–5 zdaniach, z najważniejszymi szczegółami";
    maxTokens = 350;
  }

  const prompt = data.isSelection
...
Tekst:
${content}`
    : `Podsumuj ten artykuł ${lengthHint}.
...
async function openPerplexityForTab(tab, isSelection, selectionText, settings) {
  try {
    ...
    let lengthHint;
    if (settings.summaryMode === "long") {
      lengthHint = "dość szczegółowo, około 8-10 zdań";
    } else {
      lengthHint = "krótko, około 4–5 zdań";
    }

    let formatHint;
    if (settings.summaryFormat === "bullets") {
      formatHint = "zastosuj listę punktów";
    } else {
      formatHint = "zastosuj jeden zwarty akapit";
    }

    const query = `Podsumuj ten artykuł ${lengthHint}, skupiając się na najważniejszych faktach i wnioskach, ${formatHint}. Pomiń dygresje, przykłady, cytaty i linki. URL: ${data.url}. Treść artykułu: ${content}`;
    ...
    if (browser.notifications && browser.notifications.create) {
      browser.notifications.create({
        type: "basic",
        iconUrl: "icon-32.png",
        title: "Podsumowanie Perplexity",
        message: "Nie udało się otworzyć Perplexity dla tej strony."
      });
    }
  }
}
...
function extractMainContent() {
  ...
  text = text.replace(/\s*Visit our[^.]*?(hub|platform)[^.]*?FT[^.]*?(?:\.|and)\s*/gi, " ");
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "getSettings") {
    browser.storage.local.get("settings").then((res) => {
      sendResponse(res.settings || defaultSettings);
    });
    return true;
